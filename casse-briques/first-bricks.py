...
def miseAJour():
    ...
    for x in range(0, briques['GRILLE_TAILLE']):
        if (briques['grille'][x] == 0):
            # Ignorer les briques nulles
            pass  # 'pass' force la boucle à passer a recommencer (et donc incrémenter)
        
        briqueX = x * (briques['BRIQUE_LARGEUR'] + 2) + 1
        briqueY = 10;  # Hauteur des briques
        if (gb.collide.rectRect(balle['X'], balle['Y'], balle['TAILLE'], balle['TAILLE'], briqueX, briqueY, briques['BRIQUE_LARGEUR'], briques['BRIQUE_HAUTEUR'])):
            balle['VY'] *= -1
            briques['grille'][x] = 0 # Détruire la brique
            # Verifier qu'il reste encore des briques
            encoreDesBriques = False  # Si ce booléen reste faux, alors il n'y a plus de briques
            for i in range(0, briques['GRILLE_TAILLE']):
                if (briques['grille'][x] == 0):
...
def reinitialiser():
    ...
    balle['VY'] = -1  # Haut
    for i in range(0, briques['GRILLE_TAILLE']):
        briques['grille'][i] = 1 # Faire réapparaitre toutes les briques
...
briques['grille'] = [0] * briques['GRILLE_TAILLE']
